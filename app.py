...
def load_maestro(file_obj):
    df = pd.read_excel(file_obj, dtype={'codart': str, 'ean': str})
    df.columns = df.columns.str.strip()
    rename = {}
    for col in df.columns:
        cl = col.lower()
        if cl == 'fecha':      rename[col] = 'fecha'
        if cl == 'comparable': rename[col] = 'COMPARABLE'
        if cl == 'vender_en':  rename[col] = 'Vender_en'
    if rename:
        df.rename(columns=rename, inplace=True)
    df['fecha'] = pd.to_numeric(df['fecha'], errors='coerce').astype('Int64')
    df['COMPARABLE'] = df['COMPARABLE'].apply(
        lambda v: '' if (v is None or (not isinstance(v, str) and pd.isna(v)) or v == 0)
                  else str(v).strip())
...
def calc_resumen(df):
    def build_block(data):
        ...
        tot26 = by26.sum()
        d26v = d26.copy(); d26v['tg'] = d26v['Vender_en'].apply(map_vend)
...
def write_data_sheet(wb, name, df):
    ...
    for ci, (lbl, width, fmt, orange) in enumerate(col_defs, 1):
        ...
        c.fill = fill(ORG if orange else BLK)
        c.font = font(WHT, True)
        c.alignment = align()
    ...
    for ri, row in enumerate(df.itertuples(index=False), 2):
        ...
        for ci, (_, _, fmt, _) in enumerate(col_defs, 1):
            ...
            col_name = str(col_defs[ci-1][0])
            if 'DIF' in col_name.upper() and isinstance(val, (int,float)) and val is not None:
...
def generar_excel(df, tipo, semana, anio):
    ...
    for name, fn in tabs:
        write_data_sheet(wb, name, fn(df))
    buf = io.BytesIO()
    wb.save(buf); buf.seek(0)
    fname = f'PESOS_{tipo}_W{semana:02d}_{anio}.xlsx'
    return buf, fname
...
.dz{border:1.5px dashed var(--border);border-radius:12px;padding:26px 18px;text-align:center;cursor:pointer;transition:all .2s;background:var(--card);position:relative;min-height:145px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px}
...
.dz-lbl{font-size:10px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--mid)}
.dz-sub{font-size:11px;color:#bbb;line-height:1.5}
.dz-opt{font-size:10px;color:#ccc;font-style:italic}
...
.dz-clr:hover{background:#f0ede4;color:var(--ink)}
...
.c-blk{background:var(--ink);color:var(--accent)}
.c-org{background:var(--orange);color:#fff}
.c-grn{background:var(--green);color:var(--green-dk)}
.c-red{background:var(--red);color:#9c0006}
.c-num{background:#f0f0ea;color:var(--ink);border:1px solid var(--border)}
...
.prog-spinner{width:36px;height:36px;border:3px solid var(--border);border-top-color:var(--ink);border-radius:50%;animation:spin .8s linear infinite;margin:0 auto 12px}
...
<main>
  <div class="week-row">
    <div class="wlbl"><label>Semana</label><input type="number" id="isem" min="1" max="53" value="{{ semana_actual }}"/></div>
    <span class="wsep">/</span>
    <div class="wlbl"><label>Año</label><input type="number" id="ianio" min="2020" max="2099" value="{{ anio_actual }}"/></div>
  </div>
  <div class="drop-grid">
    <div class="dz" id="dz-fp">
      <input type="file" id="f-fp" accept=".xlsx,.xls"/>
      <div class="dz-icon">📋</div>
      <div class="dz-lbl">Maestro Full Price</div>
      <div class="dz-sub">Arrastra aquí o haz clic</div>
    </div>
    <div class="dz" id="dz-out">
      <input type="file" id="f-out" accept=".xlsx,.xls"/>
      <div class="dz-icon">🏪</div>
      <div class="dz-lbl">Maestro Outlet</div>
      <div class="dz-sub">Arrastra aquí o haz clic</div>
  ...
  <div class="fmt-bar">
    ...
    <span class="chip c-num"># Formato €</span>
  </div>
  <div id="err"></div>
  <div id="prog">
...
function setProg(p,l){ $('prog-fill').style.width=p+'%'; if(l) $('prog-lbl').textContent=l; }
...
$('isem').addEventListener('input',upPill);
$('ianio').addEventListener('input',upPill);
upPill();

$('btn').addEventListener('click', async()=>{
  hideErr(); $('results').innerHTML='';
  $('btn').disabled=true; $('prog').style.display='block'; setProg(5,'Preparando...');

  const sem=$('isem').value, anio=$('ianio').value;
  const toProcess=Object.entries(files);
  const resultCards=[];

  try{
    for(let i=0;i<toProcess.length;i++){
      const[key,file]=toProcess[i];
      const tipo=key==='fp'?'FULL_PRICE':'OUTLET';
      setProg(10+Math.round(i/toProcess.length*80), `Generando ${tipo}...`);
      ...
      resultCards.push({key,tipo,fname,url});
      setProg(10+Math.round((i+1)/toProcess.length*80));
    }
      ...
      <div class="result-card">
        <div class="result-info">
          ...
          <div>
            ...
            <div class="result-sub">${tipo==='FULL_PRICE'?'Full Price':'Outlet'} · 10 pestañas formateadas</div>
          </div>
        </div>
...
def index():
    ...
    return render_template_string(HTML, semana_actual=semana, anio_actual=now.year)
...
def generar():
    try:
        ...
        if len(df) == 0:
            return 'El archivo está vacío o no tiene datos válidos', 400
